...
function nginx_download_and_unpack() {
    echo "Preparing NGINX:";

    local EXTRACT_DIR="${OPENSHIFT_NGINX_DIR}/usr/"
    local FILENAME="nginx-${NGINX_VERSION}.tar.gz"

    local TARGET_FILE="${OPENSHIFT_NGINX_DIR}/usr/${FILENAME}"
    local TARGET_FILE_MIMETYPE=$(file -b --mime-type "${TARGET_FILE}")

    local LFS_FILE="${OPENSHIFT_NGINX_DIR}/usr/${FILENAME}.lfs"
    ...
    for ARCHIVE_SERVER in "${ARCHIVE_SERVERS[@]}"; do
        local rc=0
        nginx_archive_download "${ARCHIVE_SERVER}/${FILENAME}" "${TARGET_FILE}" || rc=$?
        if [ $rc -eq 0 ]; then
            unpack "${TARGET_FILE}" "${EXTRACT_DIR}"
            return $?
        fi
    ...
    nginx_lfs_download "${LFS_FILE}" "${TARGET_FILE}"
    unpack "${TARGET_FILE}" "${EXTRACT_DIR}"
    return $?
}

function nginx_archive_download() {
    if [ -z "${1}" ] || [ -z "${2}" ]; then
        echo "nginx_archive_download: expected two arguments"
        exit -1
    fi
...
function nginx_lfs_download() {
    if [ -z "${1}" ] || [ -z "${2}" ]; then
        echo "nginx_lfs_download: expected two arguments"
        exit -1
    fi
